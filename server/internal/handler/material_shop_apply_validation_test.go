...
func newValidMaterialShopApplyInput() materialShopApplyInput {
	return materialShopApplyInput{
		...
		BusinessLicenseNo: "91310106MA1FY6XQ2L",
		...
		Products: []materialShopApplyProductInput{
			{
				Name:   "产品1",
				Price:  100,
				Images: []string{"https://img.example.com/p1.jpg"},
				Params: map[string]interface{}{"品牌": "A", "规格": "100x100"},
			},
			{
				Name:   "产品2",
				Price:  120,
				Images: []string{"https://img.example.com/p2.jpg"},
				Params: map[string]interface{}{"品牌": "A", "规格": "120x120"},
			},
			{
				Name:   "产品3",
				Price:  130,
				Images: []string{"https://img.example.com/p3.jpg"},
				Params: map[string]interface{}{"品牌": "A", "规格": "130x130"},
			},
			{
				Name:   "产品4",
				Price:  140,
				Images: []string{"https://img.example.com/p4.jpg"},
				Params: map[string]interface{}{"品牌": "A", "规格": "140x140"},
			},
			{
				Name:   "产品5",
				Price:  150,
				Images: []string{"https://img.example.com/p5.jpg"},
				Params: map[string]interface{}{"品牌": "A", "规格": "150x150"},
			},
		},
...
func TestValidateMaterialShopApply_RequireContactName(t *testing.T) {
	input := newValidMaterialShopApplyInput()
	input.ContactName = ""

	err := validateMaterialShopApply(&input)
	if err == nil {
		t.Fatalf("expected error for missing contactName")
	}
	if !strings.Contains(err.Error(), "联系人姓名") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestValidateMaterialShopApply_RequireValidContactPhone(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "联系人手机号格式不正确") {
		t.Fatalf("unexpected error: %v", err)
	}
}
