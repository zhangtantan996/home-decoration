...
func Setup(cfg *config.Config, dictHandler *handler.DictionaryHandler) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		admin.Use(middleware.AdminLog()) // ✅ 记录操作日志
		{
			...
			admin.GET("/legal-compliance/releases/current", settingListPerm, handler.AdminGetLegalComplianceCurrent)
			admin.GET("/legal-compliance/draft", settingListPerm, handler.AdminGetLegalComplianceDraft)
			admin.PUT("/legal-compliance/draft/documents/:slug", settingEditPerm, handler.AdminSaveLegalComplianceDraftDocument)
			admin.POST("/legal-compliance/draft/publish", settingEditPerm, middleware.RequireAdminReason(), middleware.RequireAdminReauth(), handler.AdminPublishLegalComplianceDraft)
			admin.GET("/legal-compliance/releases", settingListPerm, handler.AdminListLegalComplianceReleases)
			admin.GET("/legal-compliance/releases/:id", settingListPerm, handler.AdminGetLegalComplianceRelease)
