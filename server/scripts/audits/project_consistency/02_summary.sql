-- 项目一致性巡检汇总
-- 用法示例：
--   psql ... -v project_id=99001 -f 02_summary.sql
--   psql ... -v severity="'fatal'" -f 02_summary.sql
...
findings AS (
  SELECT 'P001' AS rule_code, 'fatal' AS severity, project_id FROM snapshot
  WHERE escrow_total = 0
    AND (status IN (0, 1, 2) OR business_status IN ('construction_quote_confirmed', 'in_progress', 'completed'))
  UNION ALL
  SELECT 'P002', 'fatal', project_id FROM snapshot
  WHERE phase_total = 0 AND business_status IN ('construction_quote_confirmed', 'in_progress', 'completed')
  UNION ALL
  SELECT 'P003', 'fatal', project_id FROM snapshot
  WHERE milestone_total = 0 AND business_status IN ('construction_quote_confirmed', 'in_progress', 'completed')
  UNION ALL
  SELECT 'P004', 'high', project_id FROM snapshot
  WHERE flow_total = 0 AND (provider_id > 0 OR construction_provider_id > 0 OR foreman_id > 0 OR business_status <> 'draft')
  UNION ALL
  SELECT 'P005', 'high', project_id FROM snapshot WHERE flow_total > 1
  UNION ALL
  SELECT 'P006', 'high', project_id FROM snapshot WHERE status = 1 AND business_status <> 'completed'
  UNION ALL
  SELECT 'P007', 'high', project_id FROM snapshot WHERE status = 1 AND COALESCE(flow_stage_one, '') NOT IN ('completed', 'archived')
  UNION ALL
  SELECT 'P008', 'high', project_id FROM snapshot
  ...
  UNION ALL
  SELECT 'P009', 'high', project_id FROM snapshot
  WHERE current_phase LIKE '%待验收%'
    AND milestone_submitted_count = 0
    AND current_phase <> '已完工待验收'
  UNION ALL
  SELECT 'P010', 'high', project_id FROM snapshot
  WHERE current_phase LIKE '%施工中%'
    AND phase_in_progress_count = 0
    AND milestone_in_progress_count = 0
  UNION ALL
  SELECT 'P011', 'high', project_id FROM snapshot
  WHERE current_phase = '已完工待验收'
    AND completion_submitted_at IS NULL
  UNION ALL
  SELECT 'P012', 'medium', project_id FROM snapshot
  WHERE current_phase = '已归档'
    AND COALESCE(inspiration_case_draft_id, 0) = 0
  UNION ALL
  SELECT 'P013', 'medium', project_id FROM snapshot
  WHERE provider_id = 0 OR provider_type IS NULL OR provider_display_seed IS NULL
  UNION ALL
  SELECT 'P014', 'medium', project_id FROM snapshot
  WHERE escrow_total > 0
    AND (
  ...
  UNION ALL
  SELECT 'P015', 'fatal', project_id FROM snapshot
  WHERE status = 1
