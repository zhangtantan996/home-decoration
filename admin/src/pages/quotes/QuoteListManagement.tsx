...
const PREREQUISITE_STATUS_META: Record<
  string,
...
const QuoteListManagement: React.FC = () => {
  ...
  const [recommendedForemen, setRecommendedForemen] = useState<
    RecommendedForeman[]
  >([]);
  ...
  const loadLibraryRows = async () => {
    const data = await adminQuoteApi.listLibraryItems({
      page: 1,
      pageSize: 200,
      status: 1,
    });
  ...
  const openDetail = async (id: number) => {
    try {
      ...
      const snapshot = safeJsonParse<Record<string, unknown>>(
        detailData.quoteList.prerequisiteSnapshotJson,
        {},
      );
      ...
      setSelectedForemanIds(
        detailData.invitations.map((item) => item.providerId),
      );
  ...
  const handleValidatePrerequisites = async () => {
    ...
    try {
      setSubmitting(true);
      const result = await adminQuoteApi.validateTaskPrerequisites(
        detail.quoteList.id,
      );
      if (result.ok) {
  ...
  const handleAppendItems = async () => {
    if (!detail) return;
    const selectedItems = libraryRows.filter((item) =>
      selectedLibraryKeys.includes(item.id),
    );
  ...
  const handleSelectForemen = async () => {
    ...
    try {
      setSubmitting(true);
      await adminQuoteApi.selectForemen(
        detail.quoteList.id,
        selectedForemanIds,
      );
  ...
  const handleGenerateDrafts = async () => {
    ...
    try {
      setSubmitting(true);
      const comparison = await adminQuoteApi.generateDrafts(
        detail.quoteList.id,
      );
  ...
  const taskColumns: ColumnsType<QuoteListSummary> = [
    ...
    },
    {
      ...
      width: 150,
      render: (value?: string) =>
        renderStatusPill(BUSINESS_STAGE_META[value || ""]),
    },
    {
      title: "前置数据",
      ...
      width: 120,
      render: (value?: string) =>
        renderStatusPill(PREREQUISITE_STATUS_META[value || "draft"]),
    },
    {
      title: "用户确认",
    ...
    },
    {
      ...
      width: 100,
      render: (value: number) => (
        <InlinePill tone="success" text={`${value}`} />
      ),
    },
    {
      title: "操作",
      ...
      render: (_value, record) => (
        <Space size={0}>
          ...
          <Button
            type="link"
            onClick={() => navigate(`/projects/quotes/compare/${record.id}`)}
          >
            对比 / 提交用户确认
          </Button>
        </Space>
    ...
    <div className="hz-page-stack">
      ...
      <ToolbarCard>
        <div className="hz-toolbar">
          ...
          />
          <Button type="primary" onClick={() => void loadQuoteTasks()}>
            筛选
          </Button>
          <Button
            onClick={() => {
        ...
        footer={
          detail ? (
            <Space style={{ width: "100%", justifyContent: "flex-end" }}>
              ...
              <Button
                onClick={() =>
                  navigate(`/projects/quotes/compare/${detail.quoteList.id}`)
                }
              >
                查看对比
              </Button>
            ...
            <div className="hz-summary-grid">
              <div className="hz-summary-card">
                ...
                <div className="hz-summary-card__value">
                  {PREREQUISITE_STATUS_META[
                    detail.quoteList.prerequisiteStatus || "draft"
                  ]?.text || "待补齐"}
                </div>
              ...
              <div className="hz-summary-card">
                ...
                <div className="hz-summary-card__value">
                  {BUSINESS_STAGE_META[detail.businessStage || ""]?.text ||
                    "未初始化"}
                </div>
                <div className="hz-summary-card__meta">
                  {detail.flowSummary || "当前报价任务尚未挂到完整业务主链"}
                </div>
              </div>
              <div className="hz-summary-card">
            ...
            <Card className="hz-panel-card">
              ...
              <Descriptions bordered column={3} size="small">
                ...
                <Descriptions.Item label="闭环阶段">
                  {renderStatusPill(
                    BUSINESS_STAGE_META[detail.businessStage || ""],
                  )}
                </Descriptions.Item>
                <Descriptions.Item label="前置数据">
                  {renderStatusPill(
                    PREREQUISITE_STATUS_META[
                      detail.quoteList.prerequisiteStatus || "draft"
                    ],
                ...
                <Descriptions.Item label="用户确认">
                  {renderStatusPill(
                    USER_CONFIRM_META[
                      detail.quoteList.userConfirmationStatus || "pending"
                    ],
                  )}
                </Descriptions.Item>
                ...
                <Descriptions.Item label="设计师服务商 ID">
                  {detail.quoteList.designerProviderId || "-"}
                </Descriptions.Item>
                <Descriptions.Item label="客户 ID">
                  {detail.quoteList.customerId || "-"}
                </Descriptions.Item>
                <Descriptions.Item label="房屋 ID">
                  {detail.quoteList.houseId || "-"}
                </Descriptions.Item>
                <Descriptions.Item label="用户 ID">
                  {detail.quoteList.ownerUserId || "-"}
                </Descriptions.Item>
                <Descriptions.Item label="币种">
                  {detail.quoteList.currency || "-"}
                </Descriptions.Item>
                ...
                <Form form={prerequisiteForm} layout="vertical">
                  ...
                  <Space style={{ width: "100%" }} size={16} align="start">
                    <Form.Item
                      ...
                      label="装修类型"
                      style={{ flex: 1 }}
                      rules={[{ required: true, message: "请填写装修类型" }]}
                    >
                    ...
                    <Form.Item
                      ...
                      label="施工范围"
                      style={{ flex: 1 }}
                      rules={[{ required: true, message: "请填写施工范围" }]}
                    >
                      <Input placeholder="例如：水电 + 泥瓦 + 油工" />
                    </Form.Item>
                  </Space>
                  <Form.Item name="serviceAreas" label="服务城市">
                    <Select
                      mode="tags"
                      tokenSeparators={[",", "，"]}
                      placeholder="例如：西安市, 咸阳市"
                    />
                  </Form.Item>
            ...
            <Card
              ...
              extra={
                <Button
                  ...
                  disabled={
                    detail.quoteList.status === "submitted_to_user" ||
                    detail.quoteList.status === "user_confirmed"
                  }
                >
                  从标准项库添加
                </Button>
                ...
                <div className="hz-quote-list">
                  {detail.items.map((item) => (
                    <div key={item.id} className="hz-quote-list__item">
                      ...
                      <div className="hz-quote-list__meta">
                        {item.categoryL1 || "未分类"} /{" "}
                        {item.categoryL2 || item.unit} / 数量 {item.quantity}
                    ...
                    description={
                      <div className="hz-quote-list" style={{ marginTop: 12 }}>
                        {recommendedForemen.map((item) => (
                          <div
                            key={item.providerId}
                            className="hz-quote-list__item"
                          >
                            <div className="hz-quote-list__title">
                              {item.providerName}
                            </div>
                            <div className="hz-quote-list__meta">
                              覆盖率 {(item.priceCoverageRate * 100).toFixed(0)}
                              % · 命中 {item.matchedItemCount} 项 / 缺失{" "}
                              {item.missingItemCount} 项
                            </div>
                            <div
                              style={{
                                marginTop: 8,
          ...
          columns={[
            {
              ...
              render: (value?: string) =>
                value ? (
                  <InlinePill tone="muted" text={value} monospace />
                ) : (
                  "-"
                ),
            },
