...
const DisputedBookings: React.FC = () => {
  ...
  const [selectedBooking, setSelectedBooking] =
    useState<DisputedBooking | null>(null);
  ...
  const handleResolveSubmit = async () => {
    try {
      ...
      const res = (await adminDisputeApi.resolve(
        selectedBooking!.id,
        values,
      )) as any;
      if (res.code === 0) {
    ...
    <div className="hz-page-stack">
            ...
            <Descriptions title="预约信息" column={2} bordered size="small">
              ...
              <Descriptions.Item label="商家">
                {selectedBooking?.providerName ||
                  detailData.provider?.displayName ||
                  detailData.provider?.companyName ||
                  "-"}
              </Descriptions.Item>
              <Descriptions.Item label="地址" span={2}>
                {detailData.booking?.address}
              </Descriptions.Item>
              <Descriptions.Item label="面积">
                {detailData.booking?.area} ㎡
              </Descriptions.Item>
              <Descriptions.Item label="意向金">
                ¥{detailData.booking?.intentFee}
              </Descriptions.Item>
            </Descriptions>
              ...
              <Timeline>
                {detailData.proposals?.map((p: any) => (
                  <Timeline.Item
                    key={p.id}
                    color={
                      p.status === 3 ? "red" : p.status === 2 ? "green" : "blue"
                    }
                  >
                    <div>
                    ...
                    <div style={{ color: "#999", fontSize: 12, marginTop: 4 }}>
                      {formatServerDateTime(p.submittedAt || p.createdAt)}
                    </div>
                  </Timeline.Item>
                ))}
              </Timeline>
      ...
      >
        ...
        >
          ...
          <div>
            拒绝次数：
            <Tag color="red">{selectedBooking?.rejectionCount} 次</Tag>
          </div>
        </div>
