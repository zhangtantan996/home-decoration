...
const APP_ENV = normalizeAppEnv(
  getConfigValue('APP_ENV') || (__DEV__ ? 'local' : 'production'),
);
...
function isNotificationRealtimeEnabled(): boolean {
  const raw = getConfigValue('ENABLE_NOTIFICATION_WS');
...
export const ENV = {
  ...
  ENABLE_NOTIFICATION_WS: isNotificationRealtimeEnabled(),
...
if (__DEV__) {
  ...
  if (Platform.OS === 'android') {
    ...
    if (isLocalhost) {
      ...
      console.log(
        '💡 提示: 当前为直连模式，请确保设备能访问该 IP，并且后端在 8080 端口运行。',
      );
    }
  }
}
