...
> 商家中心 Web 入驻入口统一为「我要入驻」，角色选择为：`设计师入驻（个人/公司） / 工长入驻（个人/公司） / 装修公司入驻 / 主材商入驻`。

### 1.5 商家入驻与登录规则（2026-03 冻结）

#### 1.5.1 入驻验证码前置校验（2026-03-09 补充）
- 商家入驻与主材商入驻统一要求：手机号验证码在**第一步**内完成真实校验。
- 用户点击第一步“下一步”时即调用后端校验，不再等到最终提交时才校验验证码。
- 若验证码错误：
  - 停留当前步骤；
  - 允许直接修改验证码或重新发送；
  - 不打断后续已填写内容。
- 若手机号未变化：返回前序步骤不要求重新验证。
- 若手机号变化：自动清空“手机号已验证”状态并重新走验证码流程。
- 驳回重提：第一步校验成功后自动回填原申请资料；手机号只读、商家子类型只读、协议需重新确认。

#### 1.5.1 登录未入驻手机号交互
- 登录提交后，若手机号无入驻记录：
  - 跳转前 Toast（1.2s）：`该手机号尚未入驻，正在为你跳转入驻页`
  - 自动跳转到入驻页并回填手机号（可编辑）
  - 入驻页顶部固定提示：`该手机号尚未入驻，请先完成入驻申请后再登录`
- 审核中/驳回用户按 `nextAction` 自动引导到进度页或重提页。

#### 1.5.2 入驻资料硬门槛
- 设计师：头像必填、从业年限必填（1-50）、案例至少3套且每套3-6图并强制案例说明。
- 工长：头像必填、施工年限必填（1-50）、工种>=1、亮点1-3、案例至少3套且每套8-12图并强制案例说明。
- 装修公司：公司执照必填，案例至少3套，报价需覆盖全包/半包。
- 主材商：联系人姓名/手机号、营业时间、地址必填；商品数量5-20，每商品需价格+参数+至少1图。

#### 1.5.3 数据展示链路
- 审核通过后，服务商扩展字段（`highlightTags/pricing/graduateSchool/designPhilosophy`）同步到 C 端详情页。
- C 端按“有值显示、空值隐藏”渲染，不展示空占位字段。
...
- 商家提交入驻申请前必须勾选同意三份条款：
  ...
  - 《平台规则》
  - 《隐私与数据处理条款》
- 服务端必须硬校验 `legalAcceptance`：
  - `accepted=true`
  - `onboardingAgreementVersion/platformRulesVersion/privacyDataProcessingVersion` 非空且长度 `1-64`
- 留痕落库要求：
  - `legal_acceptance_json`
  - `legal_accepted_at`
  - `legal_accept_source`
- 条款版本升级时，需同步更新商家端常量与接口文档，确保线上勾选版本与审计版本一致。
