...
export const formatProviderPricing = ({
  ...
  const structuredUnit = defaultStructuredUnit();
  ...
  if (structured.entries.length > 0) {
    ...
    const detail = structured.entries.map((entry) => `${entry.label} ${formatAmount(entry.amount, role, structuredUnit)}`).join(' · ');
    return buildResult({
      role,
      summary,
      detail,
